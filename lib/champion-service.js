import {
  ...
  toDifficultyLabel,
  toKebabCase
} from './champion-core.js';
import { getFeaturedGuideOverride } from './featured-guide-overrides.js';
...
export async function fetchLatestDataDragonVersion() {
  ...
  try {
    ...
    cachedVersion = Array.isArray(versions) && versions.length > 0 ? versions[0] : DEFAULT_DATA_DRAGON_VERSION;
...
export async function fetchChampionCatalog(version) {
  if (cachedCatalog && catalogVersion === version) return cachedCatalog;
  ...
  cachedCatalog = transformChampionList(data.data || {});
